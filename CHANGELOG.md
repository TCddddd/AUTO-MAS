...
- BetterGI专项 一条龙的执行层改为单次启动：战斗项与自定义配置组按左栏队列顺序在同一次运行里执行，不再分成两次启动
...
- BetterGI专项 修复一条龙右栏的战斗队伍/战斗策略留空后仍切到旧队伍或沿用旧策略、地脉花单独设置的战斗策略不生效的问题
- BetterGI专项 修复未选择首领时自动首领讨伐不执行也不提示、整轮仍显示完成的问题
