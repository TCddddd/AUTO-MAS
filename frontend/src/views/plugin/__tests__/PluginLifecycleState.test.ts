...
// ===== Type mocks (must match ../types.ts) =====
...
describe('PluginLifecycleState', () => {
  it('renders discovered state when no runtimeState', async () => {
    ...
    expect(text).toContain('已发现')
    app.unmount()
  })

  it('renders active state with success color', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({ status: 'active' }),
    })
    registerStubs(app)
    const container = dom.document.createElement('div')
    app.mount(container)
    ...
    // Check data-status attribute on the root lifecycle container
    const root = container.childNodes[0]
    expect(root.attrs['data-status']).toBe('active')
    app.unmount()
  })

  it('renders error state with failed status', async () => {
    const app = createApp(PluginLifecycleState, {
  ...
  it('renders disabled state for disposed status', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({ status: 'disposed' }),
    })
    registerStubs(app)
    const container = dom.document.createElement('div')
    app.mount(container)
    ...
    const text = collectText(container)
    expect(text).toContain('已销毁')
    const root = container.childNodes[0]
    expect(root.attrs['data-status']).toBe('disabled')
    app.unmount()
  })

  it('renders installed state for loaded status', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({ status: 'loaded' }),
    })
    registerStubs(app)
    const container = dom.document.createElement('div')
    app.mount(container)
  ...
  it('shows lifecycle phase tag', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({
        status: 'active',
    ...
    const text = collectText(container)
    expect(text).toContain('重载准备')
    app.unmount()
  })

  it('shows reload count in details', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({ reload_count: 5 }),
      showDetails: true,
    })
    registerStubs(app)
    const container = dom.document.createElement('div')
  ...
  it('shows last reload reason', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({
        last_reload_reason: 'Config changed',
      }),
      showDetails: true,
    })
    registerStubs(app)
  ...
  it('hides details when showDetails is false', async () => {
    ...
    const detailsEl = findByClass(container, 'lifecycle-details')
    expect(detailsEl).toBeNull()
    app.unmount()
  })

  it('shows error result when failed', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({
        status: 'error',
        last_error: 'Critical failure',
      }),
    })
    registerStubs(app)
    const container = dom.document.createElement('div')
    ...
    const text = collectText(container)
    expect(text).toContain('插件异常')
    expect(text).toContain('重载实例')
    expect(text).toContain('复制诊断信息')
    app.unmount()
  })

  it('emits reload when reload button clicked', async () => {
    ...
    const reloadButton = buttons.find(b => collectText(b).includes('重载'))
    expect(reloadButton).toBeTruthy()
    reloadButton.dispatchEvent('click')
    await nextTick()
    expect(onReload).toHaveBeenCalledOnce()
    app.unmount()
  })

  it('emits copyDiagnostics when copy button clicked', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({
        status: 'error',
        last_error: 'Test error',
  ...
  it('does not show error result for non-failed states', async () => {
    const app = createApp(PluginLifecycleState, {
      runtimeState: makeRuntimeState({ status: 'active' }),
    })
    registerStubs(app)
