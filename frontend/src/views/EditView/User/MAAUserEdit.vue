...
import { STAGE_QUERY_TYPE, TASK_CREATE_MODE, dispatchApi, infoApi, userApi } from '@/api'
...
const loadStageModeOptions = async () => {
  try {
    const response = await infoApi.getPlanOptions()
    if (response && response.code === 200 && response.data) {
...
const selectAndImportInfrastructureConfig = async () => {
  ...
  try {
    ...
    if (path && path.length > 0) {
      ...
      // 直接导入配置
      const result = await userApi.importInfrastructure(scriptId, userId, {
        path: path[0],
      })
...
const loadInfrastructureOptions = async () => {
  ...
  try {
    ...
    const result = await userApi.getInfrastructureOptions(scriptId, userId)
