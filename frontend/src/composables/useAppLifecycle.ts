...
// closeApp() 会在 closePromise 完成赋值前同步进入 runCloseFlow()；单独的标志确保
// 极快到达的 backend.shutdown.ready 不会被误判为“非关闭流程期间”。
...
const isClosing = (): boolean => closeInProgress

// ==================== 启动性能打点 ====================
// 使用 performance.now() 记录冷启动各阶段耗时，便于量化启动预算（目标 < 5100ms）。
// 打点通过 logger 以结构化字段输出，不引入额外依赖。
...
const recordStartupTiming = (
  ...
  const sinceOrigin = Math.round(performance.now() - startupOrigin)
  logger.info(
    `启动打点 ${stage} 耗时=${Math.round(durationMs)}ms 自起点=${sinceOrigin}ms` +
...
export function respondDialog(requestId: string, choice: boolean): boolean {
...
export function closeApp(): Promise<void> {
  ...
  // 必须先建立关闭态，再调用 async 流程。async 函数会同步执行到首个 await，
  // Service.closeApiCoreClosePost() 可能在此期间触发 ready 消息。
  closeInProgress = true
...
export function initializeAppLifecycle(): void {
  ...
  recordStartupTiming('lifecycle-init', performance.now() - initStart)
}
...
export async function connectWithRetry(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    recordStartupTiming(`connect-attempt-${attempt}`, performance.now() - attemptStart, {
      result: 'failed',
    })
    if (attempt < attempts) {
  ...
  recordStartupTiming('connect-all-failed', performance.now() - connectStart, { attempts })
  return false
}
...
watch(connectionState(), state => {
  if (state === 'open') {
    ...
    if (!firstOpenRecorded) {
      ...
      recordStartupTiming('ws-first-open', performance.now() - startupOrigin)
    }
  }
})
