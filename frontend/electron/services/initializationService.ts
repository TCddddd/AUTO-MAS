...
export class InitializationService {
  ...
  async initialize(
    ...
    try {
      ...
      // 阶段 3: 安装 uv（保留 pip 阶段标识以兼容现有前端协议）
      onProgress?.({
        stage: 'pip',
        stageIndex: 3,
        totalStages,
      ...
      const uvInstaller = new PipInstaller(this.appRoot, this.mirrorService)
      const uvResult = await uvInstaller.install(installProgress => {
        onProgress?.({
          stage: 'pip',
          stageIndex: 3,
          totalStages,
