...
// Electron 会在子 frame 中执行 preload；特权桥只属于应用顶层渲染器。
if (process.isMainFrame) {
  contextBridge.exposeInMainWorld('electronAPI', {
    ...
    installPython: (selectedMirror?: string) =>
      ipcRenderer.invoke('install-python', selectedMirror),
    ...
    installPluginBootstrap: (selectedMirror?: string) =>
      ipcRenderer.invoke('install-plugin-bootstrap', selectedMirror),
    repairRuntimeAndStart: (selectedMirror?: string) =>
      ipcRenderer.invoke('repair-runtime-and-start', selectedMirror),
    ...
    onPluginBootstrapProgress: (callback: (progress: any) => void) => {
      ipcRenderer.on('plugin-bootstrap-progress', (_, progress) => callback(progress))
    },
    removePluginBootstrapProgressListener: () => {
      ipcRenderer.removeAllListeners('plugin-bootstrap-progress')
    },
