...
def resolve_m7a_main_stage(user_config: Any) -> tuple[str, str] | None:
    ...
    if native is not None and native[0] in M7A_INSTANCE_TYPES_DAILY:
        return native
    return None
...
def resolve_m7a_ornament_stage(user_config: Any) -> str | None:
    ...
    if native is not None and native[0] == M7A_INSTANCE_TYPE_ORNAMENT:
        return native[1]
    return None
...
def resolve_m7a_eow_stage(user_config: Any) -> str | None:
    ...
    if native is not None and native[0] == M7A_EOW_INSTANCE_NAME_KEY:
        return native[1]
    return None
