...
from automas_script_hsr.runtime.log_detect import (
...
def build_sra_start_game_config(
    ...
    username_cipher = user_config._config_item_index["SRA"]["Id"].getValue(
        if_decrypt=False
    )
    password_cipher = user_config._config_item_index["SRA"]["Password"].getValue(
        if_decrypt=False
    )
...
def _resolve_sra_currency_wars_strategy(script_config) -> str:
    ...
    raw_path = str(script_config.get("SRA", "Path") or "").strip()
    if not raw_path:
