...
from .runtime import OkScriptAutoProxyTask

logger = get_logger("ok-script 调度器")

METHOD_BOOK: dict[str, type[OkScriptAutoProxyTask]] = {
    "AutoProxy": OkScriptAutoProxyTask,
}
...
class OkScriptManager(TaskExecuteBase):
    ...
    async def check(self) -> str:
        ...
        if not isinstance(script_config, OkefConfig):
            return "脚本配置类型错误, 不是 ok-script 类型"
    ...
    async def prepare(self) -> None:
        ...
        logger.success(f"{self.script_info.script_id} 已锁定，ok-script 用户配置已提取")
    ...
    async def main_task(self) -> None:
        ...
        if self.script_config is None or self.user_config is None:
            raise RuntimeError("ok-script 运行配置未初始化")
    ...
    async def on_crash(self, e: Exception) -> None:
        self.script_info.status = "异常"
        logger.exception(f"ok-script 任务出现异常: {e}")
        ...
        try:
            ...
            logger.exception("on_crash 写回 ok-script UserConfig 失败，放弃本次状态变更")

        try:
            await Config.send_websocket_message(
                id=self.task_info.task_id,
                type="Info",
                data={"Error": f"ok-script 任务出现异常: {e}"},
            )
        except Exception as send_error:
            logger.exception(f"发送 ok-script 调度器异常通知失败: {send_error}")
