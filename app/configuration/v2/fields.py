...
def is_virtual_model_field(model_field: FieldInfo) -> bool:
    ...
    alias = get_origin(ann)
    if isinstance(alias, TypeAliasType):
        ann = alias.__value__
    elif isinstance(ann, TypeAliasType):
        ann = ann.__value__
    if get_origin(ann) is Annotated and any(
        isinstance(m, VirtualDecl) for m in get_args(ann)[1:]
    ):
        return True
    return any(isinstance(m, VirtualDecl) for m in getattr(model_field, "metadata", ()))
...
def is_trigger_model_field(model_field: FieldInfo) -> bool:
    ann = getattr(model_field, "annotation", None)
    if get_origin(ann) is Annotated and any(
        isinstance(m, TriggerDecl) for m in get_args(ann)[1:]
    ):
        return True
    return any(isinstance(m, TriggerDecl) for m in getattr(model_field, "metadata", ()))
